...
const InvoiceGenerator = ({ defaultCountry = 'usa' }) => {
  ...
  const generatePDF = async () => {
    ...
    try {
      // Temporarily make the invoice visible for capture
      const invoiceElement = invoiceRef.current.parentElement;
  ...
  // Send invoice via email using Gmail API
  const sendViaEmail = async () => {
    ...
    try {
      // Show loading state
      const sendButton = document.querySelector('button[onclick*="sendViaEmail"]');
      if (sendButton) {
      ...
      const subject = `Invoice ${invoiceData.invoiceNumber} from ${invoiceData.senderName || 'Your Business'}`;
      const body = 
        `Dear ${invoiceData.clientName || 'Customer'},\n\n` +
        ...
        `Best regards,\n${invoiceData.senderName || 'Your Business'}`;
      ...
      try {
        ...
        // Also download PDF for user's records
        pdf.save(fileName);
      } catch (gmailError) {
        ...
        const mailtoLink = `mailto:${invoiceData.clientEmail}?subject=${encodedSubject}&body=${encodedBody}`;
      ...
      if (sendButton) {
        ...
        sendButton.textContent = 'Send to Recipient';
      }
    }
  };
  ...
  return (
    <div className="container mx-auto">
      {/* Top AdSense Banner */}
      <div className="mb-6">
        <AdSenseBanner slot={ADSENSE_CONFIG.topBannerSlot} format="horizontal" />
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="mt-6">
          <AdSenseBanner slot={ADSENSE_CONFIG.bottomBannerSlot} format="horizontal" />
        </div>
      </div>
